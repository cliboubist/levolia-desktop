...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\levolia" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\levolia-agent" } else { "$env:LOCALAPPDATA\levolia\levolia-agent" }),
...
if ($PSBoundParameters.ContainsKey('HermesHome')) {
    ...
    $HermesHome = ConvertTo-LongPath $(
        if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\levolia" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:HERMES_HOME) { "$env:HERMES_HOME\levolia-agent" } else { "$env:LOCALAPPDATA\levolia\levolia-agent" }
    )
}
...
$RepoUrlSsh = "git@github.com:cliboubist/levolia-desktop.git"
$RepoUrlHttps = "https://github.com/cliboubist/levolia-desktop.git"
$PythonVersion = "3.11"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/cliboubist/levolia-desktop/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/cliboubist/levolia-desktop/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/cliboubist/levolia-desktop/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Write-BootstrapMarker {
    ...
    $pinnedBranch = $Branch
    if (-not $pinnedBranch) {
        $pinnedBranch = "levolia"  # install.ps1's own default for -Branch
    }
